...
func (m *setProtocolVersion) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(m.TargetVersion)})
...
func (m *deleteROSpec) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(m.ROSpecID >> 24), byte(m.ROSpecID >> 16), byte(m.ROSpecID >> 8), byte(m.ROSpecID)})
...
func (m *deleteAccessSpec) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(m.AccessSpecID >> 24), byte(m.AccessSpecID >> 16), byte(m.AccessSpecID >> 8), byte(m.AccessSpecID)})
...
func (p *antennaID) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(*p >> 8), byte(*p)})
...
func (p *firstSeenUTC) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(*p >> 56), byte(*p >> 48), byte(*p >> 40), byte(*p >> 32), byte(*p >> 24), byte(*p >> 16), byte(*p >> 8), byte(*p)})
...
func (p *epc96) EncodeFields(w io.Writer) error {
	_, err :=
		w.Write([]byte{p.EPC[0], p.EPC[1], p.EPC[2], p.EPC[3], p.EPC[4], p.EPC[5], p.EPC[6], p.EPC[7], p.EPC[8], p.EPC[9], p.EPC[10], p.EPC[11]})
...
func (p *c1G2SingulationDetails) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		...
		byte(p.NumEmptySlots >> 8), byte(p.NumEmptySlots)})
...
func (p *receiveSensitivityTableEntry) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.TableIndex >> 8), byte(p.TableIndex),
		byte(p.ReceiveSensitivity >> 8), byte(p.ReceiveSensitivity)})
...
func (p *gpioCapabilities) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.NumGPIs >> 8), byte(p.NumGPIs),
		byte(p.NumGPOs >> 8), byte(p.NumGPOs)})
...
func (p *llrpCapabilities) EncodeFields(w io.Writer) error {
	_, err :=
		w.Write([]byte{byte(p.LLRPCapabilitiesFlags), p.MaxPriorityLevelSupported,
			...
			byte(p.MaxNumOpSpecsPerAccessSpec >> 24), byte(p.MaxNumOpSpecsPerAccessSpec >> 16), byte(p.MaxNumOpSpecsPerAccessSpec >> 8), byte(p.MaxNumOpSpecsPerAccessSpec)})
...
func (p *transmitPowerLevelTableEntry) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.Index >> 8), byte(p.Index),
		byte(p.TransmitPowerValue >> 8), byte(p.TransmitPowerValue)})
...
func (p *perAntennaReceiveSensitivityRange) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.AntennaID >> 8), byte(p.AntennaID),
		byte(p.ReceiveSensitivityIndexMin >> 8), byte(p.ReceiveSensitivityIndexMin),
		byte(p.ReceiveSensitivityIndexMax >> 8), byte(p.ReceiveSensitivityIndexMax)})
...
func (p *gpiTriggerValue) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		...
		byte(p.Timeout >> 24), byte(p.Timeout >> 16), byte(p.Timeout >> 8), byte(p.Timeout)})
...
func (p *tagObservationTrigger) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.TagObservationTriggerType), 0x00,
...
func (p *rfSurveySpecStopTrigger) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.RFSurveySpecStopTriggerType),
		byte(p.Duration >> 24), byte(p.Duration >> 16), byte(p.Duration >> 8), byte(p.Duration),
		byte(p.N >> 24), byte(p.N >> 16), byte(p.N >> 8), byte(p.N)})
...
func (p *accessSpecStopTrigger) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.AccessSpecStopTriggerType),
		byte(p.OperationCountValue >> 8), byte(p.OperationCountValue)})
...
func (p *gpoWriteData) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(p.GPOPort >> 8), byte(p.GPOPort), b2b(p.GPOData)})
...
func (p *keepAliveSpec) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.KeepAliveTriggerType),
		byte(p.Interval >> 24), byte(p.Interval >> 16), byte(p.Interval >> 8), byte(p.Interval)})
...
func (p *antennaProperties) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{b2b(p.AntennaConnected),
		byte(p.AntennaID >> 8), byte(p.AntennaID),
		byte(p.AntennaGain >> 8), byte(p.AntennaGain)})
...
func (p *rfTransmitter) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.HopTableID >> 8), byte(p.HopTableID),
		byte(p.ChannelIndexInFixedFrequencyTable >> 8), byte(p.ChannelIndexInFixedFrequencyTable),
		byte(p.TransmitPowerTableIndex >> 8), byte(p.TransmitPowerTableIndex)})
...
func (p *gpiPortCurrentState) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(p.GPIPort >> 8), byte(p.GPIPort), b2b(p.GPIPortEnabled), byte(p.GPIState)})
...
func (p *frequencyRSSILevelEntry) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		...
		byte(p.Bandwidth >> 24), byte(p.Bandwidth >> 16), byte(p.Bandwidth >> 8), byte(p.Bandwidth), byte(p.AverageRSSI), byte(p.PeakRSSI)})
...
func (p *eventNotificationState) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(p.ReaderEventType >> 8), byte(p.ReaderEventType), b2b(p.NotificationEnabled)})
...
func (p *gpiEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(p.GPIPort >> 8), byte(p.GPIPort), b2b(p.GPIEvent)})
...
func (p *roSpecEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.ROSpecEventType),
		byte(p.ROSpecID >> 24), byte(p.ROSpecID >> 16), byte(p.ROSpecID >> 8), byte(p.ROSpecID),
		byte(p.PreemptingROSpecID >> 24), byte(p.PreemptingROSpecID >> 16), byte(p.PreemptingROSpecID >> 8), byte(p.PreemptingROSpecID)})
...
func (p *reportBufferOverflowErrorEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{})
	return errors.Wrap(err, "failed to write fields for ParamReportBufferOverflowErrorEvent")
}
...
func (p *rfSurveyEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.RFSurveyEventType),
		byte(p.ROSpecID >> 24), byte(p.ROSpecID >> 16), byte(p.ROSpecID >> 8), byte(p.ROSpecID)})
...
func (p *antennaEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.AntennaEventType),
		byte(p.AntennaID >> 8), byte(p.AntennaID)})
...
func (p *connectionCloseEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{})
	return errors.Wrap(err, "failed to write fields for ParamConnectionCloseEvent")
}
...
func (p *fieldError) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.FieldIndex >> 8), byte(p.FieldIndex),
		byte(p.ErrorCode >> 8), byte(p.ErrorCode)})
...
func (p *c1G2LLRPCapabilities) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2CapabilitiesFlags),
		byte(p.MaxNumSelectFiltersPerQuery >> 8), byte(p.MaxNumSelectFiltersPerQuery)})
...
func (p *uhfc1G2RFModeTableEntry) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		...
		byte(p.StepTariTime >> 24), byte(p.StepTariTime >> 16), byte(p.StepTariTime >> 8), byte(p.StepTariTime), byte(p.SpectralMask)})
...
func (p *c1G2TagInventoryStateAwareFilterAction) EncodeFields(w io.Writer) error {
	_, err :=
		w.Write([]byte{byte(p.C1G2TagInventoryTarget), byte(p.C1G2TagInventoryStateAwareFilterAction)})
...
func (p *c1G2Read) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		byte(p.AccessPassword >> 24), byte(p.AccessPassword >> 16), byte(p.AccessPassword >> 8), byte(p.AccessPassword), byte(p.C1G2MemoryBank),
		byte(p.WordAddress >> 8), byte(p.WordAddress),
		byte(p.WordCount >> 8), byte(p.WordCount)})
...
func (p *c1G2Kill) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		byte(p.KillPassword >> 24), byte(p.KillPassword >> 16), byte(p.KillPassword >> 8), byte(p.KillPassword)})
...
func (p *c1G2LockPayload) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.LockPrivilege), byte(p.LockData)})
...
func (p *c1G2WriteOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2WriteOpSpecResultType),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		byte(p.WordsWritten >> 8), byte(p.WordsWritten)})
...
func (p *c1G2KillOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2KillResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID)})
...
func (p *c1G2LockOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2LockResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID)})
...
func (p *c1G2BlockEraseOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2BlockEraseResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID)})
...
func (p *c1G2BlockWriteOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2BlockWriteResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		byte(p.WordsWritten >> 8), byte(p.WordsWritten)})
...
func (p *specLoopEvent) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		byte(p.ROSpecID >> 24), byte(p.ROSpecID >> 16), byte(p.ROSpecID >> 8), byte(p.ROSpecID),
		byte(p.LoopCount >> 24), byte(p.LoopCount >> 16), byte(p.LoopCount >> 8), byte(p.LoopCount)})
...
func (p *c1G2Recommission) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		byte(p.KillPassword >> 24), byte(p.KillPassword >> 16), byte(p.KillPassword >> 8), byte(p.KillPassword), byte(p.C1G2RecommissionFlags)})
...
func (p *c1G2GetBlockPermalockStatus) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.OpSpecID >> 8), byte(p.OpSpecID),
		...
		byte(p.BlockRange >> 8), byte(p.BlockRange)})
...
func (p *c1G2RecommissionOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2RecommissionResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID)})
...
func (p *c1G2BlockPermalockOpSpecResult) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{byte(p.C1G2BlockPermalockResult),
		byte(p.OpSpecID >> 8), byte(p.OpSpecID)})
...
func (p *rfSurveyFrequencyCapabilities) EncodeFields(w io.Writer) error {
	_, err := w.Write([]byte{
		...
		byte(p.MaxFrequency >> 24), byte(p.MaxFrequency >> 16), byte(p.MaxFrequency >> 8), byte(p.MaxFrequency)})
